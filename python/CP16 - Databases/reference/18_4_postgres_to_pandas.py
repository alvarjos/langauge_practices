...
#including one specifically for working with sql.
# use pandas' read_sql_query() function. give it the query you want to send, and the engine object that has your connection
# to the postgres database. It will return a dataframe.

df_from_postgres = pd.read_sql_query(query, engine)

print(df_from_postgres)

# now you could do anything you want with a pandas dataframe
# e.g. print out the first_name column:
print(df_from_postgres["first_name"])
...
df_from_postgres2 = pd.read_sql_query(query, engine, params=(last_name_example,))

print(df_from_postgres2)
