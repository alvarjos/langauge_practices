...
class DogPerson(Person):
    ...
    def introduce_yourself(self): # method. Self MUST be a parameter
        print(f"Hello, my name is {self.name} and my hobby is {self.hobby}, and my dog's breed is {self.dog}") # get to the instance parameters by referencing self.

# make a person and a dogperson.

person1 = Person("Angel", "Swimming")
dogPerson = DogPerson("Sally", "Eating Sushi", "Great Pyranese")

# run introduce_yourself() on each of the objects.

person1.introduce_yourself()
dogPerson.introduce_yourself()